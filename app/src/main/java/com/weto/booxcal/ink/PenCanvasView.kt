...
class PenCanvasView @JvmOverloads constructor(
    ...
    private val renderer = StrokeRenderer(context)

    /** The app's paint, shared with the renderer: the live tail and the marker path use it. */
    private val paint: Paint get() = renderer.paint

    private fun displayWidth(stroke: Stroke): Float = renderer.displayWidth(stroke)
    private fun applyPaint(tool: PenTool, width: Float, colorArgb: Int) = renderer.applyPaint(tool, width, colorArgb)
    private fun strokePath(points: List<InkPoint>): Path = renderer.strokePath(points)
    private fun drawPencilSegment(canvas: Canvas, a: InkPoint, b: InkPoint, base: Float, texture: Int, sensitivity: Float, colorArgb: Int) =
        renderer.drawPencilSegment(canvas, a, b, base, texture, sensitivity, colorArgb)
    private fun drawBallpointSegment(canvas: Canvas, a: InkPoint, b: InkPoint, base: Float, sensitivity: Float, colorArgb: Int) =
        renderer.drawBallpointSegment(canvas, a, b, base, sensitivity, colorArgb)
    private fun drawBallpointBlob(canvas: Canvas, first: InkPoint, second: InkPoint, base: Float, colorArgb: Int) =
        renderer.drawBallpointBlob(canvas, first, second, base, colorArgb)

    private fun drawStroke(stroke: Stroke) {
        val canvas = bitmapCanvas ?: return
        renderer.drawStroke(canvas, stroke, screenMatrix())
    }
    ...
    companion object {
        ...
        const val BASE_WIDTH_MIN = StrokeRenderer.BASE_WIDTH_MIN
        const val BASE_WIDTH_MAX = StrokeRenderer.BASE_WIDTH_MAX

        /** Ancho del subrayador para un grosor base del cono (trazos de antes). */
        fun markerWidth(base: Float): Float = StrokeRenderer.markerWidth(base)
